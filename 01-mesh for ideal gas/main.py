...
for i in range(I_max + 1):
    y_lst[i] = u_mesh_1[N_max, i]

for i in range(1, I_max + 1):
    x_lst[i] = x_lst[i-1] + h

plt.title('Last time layer for rho*S')
plt.xlabel('channel length coordinate')
plt.ylabel('rho*S value')
plt.grid()
plt.plot(x_lst, y_lst)
plt.show()

for i in range(I_max + 1):
    y_lst[i] = u_mesh_2[N_max, i]

plt.title('Last time layer for rho*v*S')
plt.xlabel('channel length coordinate')
plt.ylabel('rho*v*S value')
plt.grid()
plt.plot(x_lst, y_lst)
plt.show()

for i in range(I_max + 1):
    y_lst[i] = u_mesh_3[N_max, i]

plt.title('Last time layer for rho*energy*S')
plt.xlabel('channel length coordinate')
plt.ylabel('rho*energy*S value')
plt.grid()
plt.plot(x_lst, y_lst)
plt.show()
...
for i in range(I_max + 1):
    y_lst[i] = u_mesh_1[0, i]

for i in range(1, I_max + 1):
    x_lst[i] = x_lst[i-1] + h

plt.title('Zero time layer for rho*S')
plt.xlabel('channel length coordinate')
plt.ylabel('rho*S value')
plt.grid()
plt.plot(x_lst, y_lst)
...
for i in range(I_max + 1):
    y_lst[i] = u_mesh_1[1, i]

plt.title('First time layer for rho*S')
plt.xlabel('channel length coordinate')
plt.ylabel('rho*energy*S value')
plt.grid()
plt.plot(x_lst, y_lst)
